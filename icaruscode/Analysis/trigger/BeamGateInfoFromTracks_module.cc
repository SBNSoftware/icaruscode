 ...
 * 
 * The time of the gates created by this module is driven by the time scale of
 * the input `anab::T0`. LArSoft prescribes this time being specified in
 * nanoseconds.
 * The Pandora pattern recognition algorithm is currently able to tag a track
 * with a time that is ultimately relative to the "trigger offset", i.e. a
 * fixed time from the start of the TPC waveform, and also a fixed time from
 * the start of the electronics time scale. In a perfectly aligned setup, this
 * is equivalent to the trigger time in real data, and to the simulation start
 * time in simulated data (also beam gate time, for the simulation samples where
 * this concept is well defined).
 * 
 * 
 */
  ...
  (art::Event& event, art::ProcessingFrame const&)
{
  ...
  for (auto const& [ iParticle, particlePtr ]: util::enumerate(particles)) {
    ...
    nanoseconds const t0 { t0Ptr->Time() };
    
    // time conversion is currently redundant here, left as documentation
    sim::BeamGateInfo gate {
        (t0 + fGateStartOffset).convertInto<nanoseconds>().value()  // start
