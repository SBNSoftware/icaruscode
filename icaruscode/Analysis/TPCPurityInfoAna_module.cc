...
void ana::TPCPurityInfoAna::beginJob()
{
  ...
  fPurityTuple = tfs->make<TNtuple>("purityTuple","Purity Tuple","run:subrun:ev:cryo:tpc:att:err");
...
void ana::TPCPurityInfoAna::analyze(art::Event const& e)
{
  ...
  for(auto const& pinfo : purityInfoVec){
    ...
    fPurityTuple->Fill(e.run(),e.subRun(),e.event(),pinfo.Cryostat,pinfo.TPC,pinfo.Attenuation,pinfo.FracError);
  }
