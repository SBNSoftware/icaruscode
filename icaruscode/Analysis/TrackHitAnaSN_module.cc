...
std::vector<std::unique_ptr<ITrackHistogramTool>> fTrackHistogramToolVec;
...
for (auto& trackHistTool : fTrackHistogramToolVec) trackHistTool->initializeHists(tfs, "AllTracks");
...
void TrackHitAna::reconfigure(fhicl::ParameterSet const& p)
{
    ...
    fTrackProducerLabelVec      = p.get< std::vector<art::InputTag> >("TrackProducerLabel",      std::vector<art::InputTag>() = {"pandoraTrackGaus"});
    fWireProducerLabelVec       = p.get< std::vector<art::InputTag> >("WireProducerLabel",       std::vector<art::InputTag>() = {"caldata"});

    fUseOnlyTrackHits           = p.get< bool >("UseOnlyTrackHits",true);   // BH NOTE: is the second parameter still the "default" above?
   ...
   // Implement the tools for handling the responses
    const std::vector<fhicl::ParameterSet>& trackHistogramToolVec = p.get<std::vector<fhicl::ParameterSet>>("TrackHistogramToolList");
    
    for(auto& trackHistogramTool : trackHistogramToolVec)
        fTrackHistogramToolVec.push_back(art::make_tool<ITrackHistogramTool>(trackHistogramTool));
...
void TrackHitAna::analyze(const art::Event& event)
{
    ...
    {
std::cout << " hit labelvec " << fHitProducerLabelVec[0] << std::endl;
	    ...
	    /////////////////////// BH BEGIN
	    // BH: get tracks and the assocation of the hits to tracks [only run on the first track label given...]
	    art::Handle< std::vector<recob::Track> > trackHandle;
	    std::vector< art::Ptr<recob::Track> > trkVec;
//std::cout << " associating labelvec " << fTrackProducerLabelVec[0] << std::endl;
	    if( event.getByLabel(fTrackProducerLabelVec[0], trackHandle) ){
	      art::fill_ptr_vector(trkVec, trackHandle);
	    }

	    art::FindManyP<recob::Hit> fmhit(trackHandle, event, fTrackProducerLabelVec[0]);
		...
		for( auto const& iHitPtr : theseHits ){
	      ...
	      // With unordered_set:
	      // Now that we have the unique set, fill the vector
...
for(auto& trackHistTool : fTrackHistogramToolVec)  {

trackHistTool->fillHistograms(event);
	      }
        }
    }
