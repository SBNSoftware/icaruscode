...
recob::MCSFitResult TrajectoryMCSFitterICARUS::fitMcs(const recob::Cluster& traj, int pid, bool momDepConst) const {

   std::cout << " traj npoints " << traj.NHits() << std::endl;
   std::cout << " traj lenght " << traj.Length() << std::endl;
   GetOptimalSegLen(1000,traj.NHits(),2,traj.Length());
...
double sigma=0.7; //to be replaced by computed D3P when ready!
//double cosa=1;
