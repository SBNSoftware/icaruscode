...
namespace daq {
/**
 *  @brief  TPCDecoderCanny class definiton
 */
class TPCDecoderCanny : virtual public IDecoderFilter
{
public:
    /**
     *  @brief  Constructor
     ...
     *  @param  pset
     */
    explicit TPCDecoderCanny(fhicl::ParameterSet const &pset);
...
TPCDecoderCanny::TPCDecoderCanny(fhicl::ParameterSet const &pset)
{
    this->configure(pset);
...
TPCDecoderCanny::~TPCDecoderCanny()
{
}

//------------------------------------------------------------------------------------------------------------------------------------------
void TPCDecoderCanny::configure(fhicl::ParameterSet const &pset)
{
...
void TPCDecoderCanny::process_fragment(detinfo::DetectorClocksData const&,
...
//    size_t nChannelsPerFragment = nBoardsPerFragment * nChannelsPerBoard;
    ...
    for(size_t board = 0; board < boardIDVec.size(); board++)
    {
        ...
        {
            mf::LogInfo("TPCDecoderCanny") << " Asking for board beyond number in fragment, want board " << board << ", maximum is " << nBoardsPerFragment << std::endl;
            continue;
        }
        ...
        for(size_t chanIdx = 0; chanIdx < nChannelsPerBoard; chanIdx++)
        {
            ...
            switch(fFilterModeVec[plane][0])
            {
                ...
                case 'm' :
                    ...
                    break;
                default:
                    std::cout << "***** FOUND NO MATCH FOR TYPE: " << fFilterModeVec[plane] << ", plane " << plane << " DURING INITIALIZATION OF FILTER FUNCTIONS IN TPCDecoderCanny" << std::endl;
                    break;
            }
...
//    }
    ...
    mf::LogInfo("TPCDecoderCanny") << "    *totalTime: " << totalTime << ", pedestal: " << pedestalTime << ", noise: " << denoiseTime << ", ped cor: " << cohPedSubTime << std::endl;

    return;
}


DEFINE_ART_CLASS_TOOL(TPCDecoderCanny)
