...
class shprFilter : shprIFilter
{
public:
    explicit shprFilter(const fhicl::ParameterSet& pset);
    ...
    const std::vector<std::complex<double>>& getResponseVec() const override {return fFilterVec;}
...
private:
    ...
    std::vector<std::complex<double>> fFilterVec;
};
    
//----------------------------------------------------------------------
// Constructor.
shprFilter::shprFilter(const fhicl::ParameterSet& pset)
{
    configure(pset);
}
    
shprFilter::~shprFilter()
{
    return;
}
    
void shprFilter::configure(const fhicl::ParameterSet& pset)
{
...
void shprFilter::setResponse(size_t numBins, double correct3D, double timeScaleFctr)
{
    ...
    for(size_t bin = 0; bin <= numBins/2 + 1; bin++)
    {
        ...
        fFilterVec.push_back(std::complex<double>(f, 0.));
    }
...
void shprFilter::outputHistograms(art::TFileDirectory& histDir) const
{
    ...
    for(int bin = 0; bin < numBins; bin++)
    {
        ...
        hist->Fill(freq, fFilterVec.at(bin).real(), 1.);
    }
...
DEFINE_ART_CLASS_TOOL(shprFilter)
}
