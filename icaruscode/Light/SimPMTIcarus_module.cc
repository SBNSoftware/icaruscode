...
namespace opdet{
  ...
  // Initialize member data here.
  {
    ...
    if(fQE>1.0001)
      std::cout << "WARNING: Quantum efficiency set in fhicl file " << temp_fQE
		<< " seems to be too large!"
		<< " Final QE must be equal or smaller than the"
		<< " scintillation pre scale applied at simulation time."
					  ...
					  std::vector<raw::OpDetWaveform>& output_opdets)
  {
    ...
    //auto const* TimeService  = lar::providerFrom< detinfo::DetectorClocksService >();
    //double trig_time = TimeService->TriggerTime()-fTriggerOffsetPMT;
    bool in_pulse=false;
  ...
  void SimPMTIcarus::produce(art::Event & e)
  {
    ...
    if(!pmtHandle.isValid()){
      std::cout << "Did not find any G4 photons from producer." << std::endl;
    }
