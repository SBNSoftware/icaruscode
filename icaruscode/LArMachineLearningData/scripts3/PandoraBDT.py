...
def DrawVariables(X, Y):
    # BH BH BH: if I ever want to use this fuction, will require some tinkering...
    plot_colors = ['r', 'g', 'b']
    plot_step = 1.0
...
def WriteXmlFile(filePath, adaBoostClassifer):
    ...
    with open(filePath, "a") as modelFile:
        ...
        WriteXmlFeature(modelFile, 'VertexId', 'Name', indentation) # BH BH BH: changed BeamParticleID to VertexID
...
def FindOptimalSignificanceCut(bdtModel, X_train, Y_train, parameters):
    ...
    nSigEntries = np.sum(sigEntries)
    nBkgEntries = np.sum(bkgEntries)
    optimalSignif = 0
    optimalSigEff = 0
    optimalBkgRej = 0
    optimalBinCut = 0
    ...
    for binCut in range(0, parameters['nBins']):
        nSigPassing = np.sum(sigEntries[binCut:])
        nBkgPassing = np.sum(bkgEntries[binCut:])
        ...
        if (signif > optimalSignif):
            nBkgFailing = np.sum(bkgEntries[:binCut])
...
def PlotBdtScores(bdtModel, X_test, Y_test, theRegionOrVertex, parameters):
    # BH BH BH: removed momentum
    # Testing BDT Using Remainder of Training Sample
    test_results = bdtModel.decision_function(X_test)
    fig, ax = plt.subplots()

    ax.set_title("Vertex finding BDT: " + str(theRegionOrVertex))
    ...
    for i, n, g in zip(parameters['SignalDefinition'], parameters['ClassNames'], parameters['PlotColors']):
        entries, bins, patches = ax.hist(test_results[Y_test == i],
                                         ...
                                         edgecolor='k')
        if i == 1:
            nEntries = np.sum(entries)
            nEntriesPassing = np.sum(entries[parameters['OptimalBinCut']:])
            sigEff = nEntriesPassing/nEntries
        elif i == 0:
            nEntries = np.sum(entries)
            nEntriesFailing = np.sum(entries[:parameters['OptimalBinCut']])
            bkgRej = nEntriesFailing/nEntries

    plt.text(0.5, 0.9, "Sig Eff {:.4%}, \nBkg Rej {:.4%}, \nScore Cut {:.2}".format(sigEff,bkgRej,parameters['OptimalScoreCut']),
    ...
    plt.yscale('log')
    x1, x2, y1, y2 = plt.axis()
    plt.axis((x1, x2, y1, y2 * 1.35))
    plt.legend(loc='upper right')
    plt.ylabel('Samples')
    plt.xlabel('Score')
    plt.tight_layout()
    plt.savefig('TrainingBdt_NTrees_' + str(parameters['nTrees']) + '_TreeDepth_' + str(parameters['TreeDepth']) + '_VertexFindingBDT-' + str(theRegionOrVertex) + '.pdf')

def PlotBdtScoresTrainTest(bdtModel, X_train, Y_train, X_test, Y_test, theRegionOrVertex, parameters):
    # BH BH BH: copied from the original PlotBdtScores and am also adding the training stuff
    # Testing BDT Using Remainder of Training Sample
    train_results = bdtModel.decision_function(X_train)
    test_results = bdtModel.decision_function(X_test)
    fig, ax = plt.subplots()
    ...
    n_test = np.zeros(2)
    n_test[0] = np.sum(test_results[Y_test==0])
    n_test[1] = np.sum(test_results[Y_test==1])

    n_train = np.zeros(2)
    n_train[0] = np.sum(train_results[Y_train==0])
    n_train[1] = np.sum(train_results[Y_train==1])

    for i, n, g, h in zip(parameters['SignalDefinition'], parameters['ClassNames'], parameters['PlotColors'], parameters['EdgeColors']):
        entries, bins, patches = ax.hist(test_results[Y_test == i],
                                         bins = parameters['nBins'],
                                         range = (-1,1),
                                         facecolor = g,
    ...
    area_wts = np.zeros(len(train_results))
    area_wts[ Y_train==0 ] = n_test[0]/n_train[0]
    area_wts[ Y_train==1 ] = n_test[1]/n_train[1]
    for i, n, g, h in zip(parameters['SignalDefinition'], parameters['ClassNames'], parameters['PlotColors'], parameters['EdgeColors']):
        entries_tr, bins_tr, patches_tr = ax.hist(train_results[Y_train == i],
                                         bins = parameters['nBins'],
                                         range = (-1,1),
                                         #weights = area_wts[Y_train == i],
                                         histtype = 'step',
                                         facecolor = g,
                                         label='%s' % n,
                                         edgecolor=h,
    ...
    plt.xlabel('Score')
    plt.tight_layout()
    plt.savefig('TrainingBdt_NTrees_' + str(parameters['nTrees']) + '_TreeDepth_' + str(parameters['TreeDepth']) + '_VertexFindingBDT-' + str(theRegionOrVertex) + '-TestAndTrain.pdf')
