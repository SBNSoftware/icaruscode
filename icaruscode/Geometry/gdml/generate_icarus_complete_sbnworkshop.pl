...
$TPC_x    =     $MaxDrift+ 3*$TPCWirePlane_x + 3*$WirePlaneSpacing + $CPA_x;
#$TPC_x    =     150.0;
$TPC_y    =     390.0;
$TPC_z    =     1960.0;

$CommonWireLength = 4640 * $UWire_zpitch; #364.90;
...
$TPCActive_z    =     1795.5;
    ...
    $TPCActive_z = (2 * 480 + 4640 + 2) * $UWire_zpitch;
...
$ExpHall_y = 1160.0;  #building underground
...
$posOverburden_y 	=	$WarmVessel_y/2 + $Overburden_y/2 + (820 - $WarmVessel_y/2) ; #warm vessel_y + overburden_y + distance between overburden and warm vessel (taken from the drawings)
...
-->
   ...
   <position name="posWarmVesselInDetEncl" unit="cm" x="$WarmVesselInDetEncl_x" y="$WarmVesselInDetEncl_y" z="$WarmVesselInDetEncl_y"/>
   <position name="posExpHallInWorld" unit="cm" x="0" y="@{[Building_y + $ExpHall_y/2]}" z="0"/>
...
#<position name="posExpHallInWorld" unit="cm" x="0" y="@{[$Hall_y/4 - $DetEnc_y/4 + $WarmVessel_x/4]}" z="0"/>
  #<position name="posWarmVesselInDetEncl" unit="cm" x="0" y="@{[-DetEnc_y/2 - 60]}" z="0"/>
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++++++++++++++++++++++++++++++++++++ gen_Materials +++++++++++++++++++++++++++++++++++++
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
EOF
	...
	print("$i+$NumberCommonUWires+$NumberCornerUWires $zpos $ypos \n");
...
#Common Wires
   ...
   $zpos = -$TPCActive_z/2 +  $CommonWireLength * $CosUAngle;
...
print CRYO <<EOF;
    <volume name="volCryostat">
     ...
     <physvol>
       ...
       <position name="posPMTPlane" unit="cm" x="@{[-$TPC_x - $PMTPlane_x/2]}" y="0" z="0" />
     </physvol>
...
<solids>
    ...
    <box name="DirtWorld" lunit="cm" 
      x="$World_x" 
      y="@{[$World_y/2 + $ExpHall_y - $Overburden_y/3]}" 
      z="$World_z"/>
EOF
...
# World structure: For the moment it is experimental hall+overburden
...
print WORLD <<EOF;
    ...
    <volume name="volDirtWorld" >
      <materialref ref="Dirt"/>
      <solidref ref="DirtWorld"/>
    </volume>
	...
	<position name="posDirtWorld" unit="cm" x="$posOverburden_x" y="@{[-$World_y/4+$ExpHall_y/2-$Overburden_y/6]}" z="$posOverburden_z"/>
      </physvol>
EOF
