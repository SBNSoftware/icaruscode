...
namespace calo {
  ...
  class ICARUSCalorimetry : public art::EDProducer {
    
  public:
    
    explicit ICARUSCalorimetry(fhicl::ParameterSet const& pset); 
  ...
  }; // class ICARUSCalorimetry

}

//-------------------------------------------------
calo::ICARUSCalorimetry::ICARUSCalorimetry(fhicl::ParameterSet const& pset)
  : EDProducer{pset},
...
//------------------------------------------------------------------------------------//
void calo::ICARUSCalorimetry::produce(art::Event& evt)
{ 
  ...
  for(size_t trkIter = 0; trkIter < tracklist.size(); ++trkIter){   
    ...
    for (size_t ipl = 1; ipl < nplanes; ++ipl){//loop over all wire planes CHANGE BY ANGELA SKIP VIEW 0
//std::cout << " ipl " << ipl << std::endl;
      ...
      for (size_t itp = 0; itp < tracklist[trkIter]->NumberTrajectoryPoints(); ++itp){
        ...
        if (tpcid.isValid) {
          try{
            //std::cout << " before trkpitch nplanes " << geom->Nplanes() << std::endl;
            fTrkPitch = lar::util::TrackPitchInView(*tracklist[trkIter], geom->Plane(ipl).View(), itp);
            //fTrkPitch = lar::util::TrackPitchInView(*tracklist[trkIter], ipl, itp);
            ...
            mf::LogWarning("ICARUSCalorimetry") << "caught exception " 
                                          << e << "\n setting pitch (C) to "
                                          << util::kBogusD;
            fTrkPitch = 0;
          }
          ...
          for (size_t ii = 0; ii<vhit.size(); ++ii){
            if (vhit[ii].key() == allHits[hits[ipl][ihit]].key()){
              ...
              if (vmeta[ii]->Index()>=tracklist[trkIter]->NumberTrajectoryPoints()){
                throw cet::exception("ICARUSCalorimetry_module.cc") << "Requested track trajectory index "<<vmeta[ii]->Index()<<" exceeds the total number of trajectory points "<<tracklist[trkIter]->NumberTrajectoryPoints()<<" for track index "<<trkIter<<". Something is wrong with the track reconstruction. Please contact tjyang@fnal.gov";
              }
	...
	PIDA = -1;
      ...
      std::cout << " KE calo " << Kin_En << std::endl;
	...
	if (channelStatus.IsBad(channel)){
	  MF_LOG_DEBUG("ICARUSCalorimetry") << "Found dead wire at Plane = " << plane 
					 << " Wire =" << iw;
...
void calo::ICARUSCalorimetry::GetPitch(art::Ptr<recob::Hit> hit, std::vector<double> trkx, std::vector<double> trky, std::vector<double> trkz, std::vector<double> trkw, std::vector<double> trkx0, double *xyz3d, double &pitch, double TickT0){
...
//    xyz = isp->second->XYZ();
  ...
  if (np>=2){//at least two points
	...
	xs->Fit("pol1","Q");
      ...
      mf::LogWarning("ICARUSCalorimetry::GetPitch") <<"Fitter failed";
      xyz3d[0] = vx[0];
    }
    delete xs;
...
namespace calo{
  ...
  DEFINE_ART_MODULE(ICARUSCalorimetry)
