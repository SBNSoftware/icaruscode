///////////////////////////////////////////////////////////////////////
// $Id: SimReadoutBoard32ICARUS.cxx,v 1.22 2010/04/23 20:30:53 seligman Exp $
//
// SimReadoutBoard32ICARUS class designed to simulate signal on a wire in the TPC
//
// katori@fnal.gov
//
...
class SimReadoutBoard32ICARUS : public art::EDProducer
{
public:
    
    explicit SimReadoutBoard32ICARUS(fhicl::ParameterSet const& pset);
    virtual ~SimReadoutBoard32ICARUS();
...
private:
   ...
   std::unique_ptr<icarus_tool::IGenNoise> fNoiseTool;         ///< Tool for generating noise
    ...
    CLHEP::HepRandomEngine&                f32NoiseEngine;
...
}; // class SimReadoutBoard32ICARUS
DEFINE_ART_MODULE(SimReadoutBoard32ICARUS)
    
//-------------------------------------------------
SimReadoutBoard32ICARUS::SimReadoutBoard32ICARUS(fhicl::ParameterSet const& pset)
    : EDProducer{pset}
    ...
    , f32NoiseEngine(art::ServiceHandle<rndm::NuRandomService>()->createEngine(*this, "HepJamesRandom", "32noise", pset, "Seed"))
...
//-------------------------------------------------
SimReadoutBoard32ICARUS::~SimReadoutBoard32ICARUS() {}
//-------------------------------------------------
void SimReadoutBoard32ICARUS::reconfigure(fhicl::ParameterSet const& p)
{
...
std::cout << " after tool constructor " << std::endl;
...
std::cout << " end reconfigure " << std::endl;
    
    return;
}
//-------------------------------------------------
void SimReadoutBoard32ICARUS::beginJob()
{
...
//-------------------------------------------------
void SimReadoutBoard32ICARUS::endJob()
{}
void SimReadoutBoard32ICARUS::produce(art::Event& evt)
{
    //--------------------------------------------------------------------
    //
...
//    const lariov::DetPedestalProvider& pedestalRetrievalAlg = art::ServiceHandle<lariov::DetPedestalService>()->GetPedestalProvider();
    ...
    if (chargeWork.size() < fNTimeSamples) throw std::range_error("SimReadoutBoard32ICARUS: chargeWork vector too small");
    
    //detector properties information
    auto const detProp = art::ServiceHandle<detinfo::DetectorPropertiesService const>()->DataFor(evt);
    
std::cout << " before next event " << std::endl;
...
std::cout << " before channel map loop " << std::endl;
...
//        if (cryostat == 0 && tpc == 0) 
std::cout << "*** Processing board " << boardPair.first << ", boardCount: " << boardCount << std::endl;
...
int chCount=0;
        ...
        for(const auto& channelPair : boardPair.second.second)
        {
            ...
            {
                throw cet::exception("SimReadoutBoard32ICARUS")
                << "\033[93m"
...
//            if (widVec.empty()) continue;
           ...
           if (widVec.size() > 0) wire = widVec[0].Wire;
            ...
            if (cryostat == 0 && tpc == 0 && plane ==0 && wire<128) 
std::cout << " channel " << channel << ", noise_factor: " << noise_factor << ", planeID: " << planeID << std::endl;

int hBoard=chCount/32;
            ...
            fNoiseTool->generateNoise(fUncNoiseEngine,
                                      ...
                                      boardPair.first,
                                      hBoard);
std::cout << " after generating noise " << std::endl;
            // Recover the SimChannel (if one) for this channel
            const sim::SimChannel* simChan = channels[channel];
for(unsigned int jn=0;jn<1 ; jn++)
 std::cout << " channel " << channel << " jn : " << jn  << " noise " << noisetmp.at(jn) << std::endl;
...
//-------------------------------------------------
void SimReadoutBoard32ICARUS::MakeADCVec(std::vector<short>& adcvec, icarusutil::TimeVec const& noisevec,
