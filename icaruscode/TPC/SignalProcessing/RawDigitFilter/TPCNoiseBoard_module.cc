...
TH1D* fGlobalRawRMSHisto;
TH1D* fGlobalCohRMSHisto;
TH1D* fGlobalIntRMSHisto;
...
std::cout << " after intrinsic resizing " << std::endl;
 ...
 double freqBin=0.6103515625; // (sampling frequency/2)/2049


int maxBoard=1000;

std::string globalrawRMSHistoName="GLOBALrawRMS";
std::string globalcohRMSHistoName="GLOBALcohRMS";
std::string globalintRMSHistoName="GLOBALLintRMS";
 fGlobalRawRMSHisto=new TH1D(globalrawRMSHistoName.c_str(),globalrawRMSHistoName.c_str(),100,0.,30.);
 fGlobalCohRMSHisto=new TH1D(globalcohRMSHistoName.c_str(),globalcohRMSHistoName.c_str(),100,0.,30.);
 fGlobalIntRMSHisto=new TH1D(globalintRMSHistoName.c_str(),globalintRMSHistoName.c_str(),100,0.,30.);

for(int jb=0;jb<maxBoard;jb++) {
std::string rawPowerHistoName="rawFFT"+std::to_string(jb);
std::cout << " raw power histo name " << rawPowerHistoName << std::endl;
...
TH1D* fRawPowerHisto=new TH1D(rawPowerHistoName.c_str(),rawPowerHistoName.c_str(),2049,0.,2049*freqBin);
fRawPowerHistos.push_back(fRawPowerHisto);
std::string cohPowerHistoName="cohFFT"+std::to_string(jb);
TH1D* fCohPowerHisto=new TH1D(cohPowerHistoName.c_str(),cohPowerHistoName.c_str(),2049,0.,2049*freqBin);
fCohPowerHistos.push_back(fCohPowerHisto);
std::string intPowerHistoName="intFFT"+std::to_string(jb);
TH1D* fIntPowerHisto=new TH1D(intPowerHistoName.c_str(),intPowerHistoName.c_str(),2049,0.,2049*freqBin);
...
std::vector<float> rp=fRawPower.at(boardFound);

//if(boardFound==334) 
 //for (unsigned int j=0; j< rp.size(); j++) 
  //if(rp[j]) std::cout << " j " << j << " rp0 " << rp[j] << std::endl;
...
//if(jv==100)
//if(jrp==34) std::cout << " before filling histo jv " << jv << " entry " <<  fRawPowerHistos[jrp]->GetBinContent(jv+1) << std::endl;
//if(jrp==34) std::cout << " jv " << jv << " tmpvect " << TMPVect[jv] << std::endl;
  fRawPowerHistos[jrp]->Fill(jv*freqBin,TMPVect[jv]);
fRawPowerHistos[jrp]->SetBinError(jv+1,fRawPowerHistos[jrp]->GetBinContent(jv+1)/sqrt(NEvents));
if(jrp==34&&fRawPowerHistos[jrp]->GetBinContent(jv+1)) std::cout << " after filling histo jv " << jv << " entry " <<  fRawPowerHistos[jrp]->GetBinContent(jv+1) << " error " <<  fRawPowerHistos[jrp]->GetBinError(jv+1) <<std::endl;
    }

fRawRMSHistos[jrp]->Fill(fRawRMS.at(jrp));
fGlobalRawRMSHisto->Fill(fRawRMS.at(jrp));
}
...
fGlobalCohRMSHisto->Fill(fCohRMS.at(jrp));
...
if(jrp==54) std::cout << " board 54 coh rms " << fCohRMSHistos[jrp]->GetMean() << std::endl;
}
fGlobalCohRMSHisto->Write();
...
fGlobalIntRMSHisto->Fill(fIntRMS.at(jrp));
    }
}
