...
namespace trkf {
   ...
   *
   * Input are a Track - or Trajectory - and various parameters: pIdHypothesis, minNumSegments, minNumAngles, segmentLength, minHitsPerSegment, minHits, nElossSteps, eLossMode, pMin, pMax, pStep, angResol, fiducialVolumeInsets, excludeVolumes, cutMode, cutAngles.
   *
   * Output is a recob::MCSFitResult, containing: PID hypothesis used in the fit, resulting momentum, momentum uncertainty, best likelihood value, vector of segment (radiation) lengths, vector of scattering angles.
   *
  ...
  public:
    struct Config {
      ...
      fhicl::Atom<double> pMin {
        Name("pMin"),
        Comment("Minimum momentum value [GeV/c] in likelihood scan."),
        0.01 };
      fhicl::Atom<double> pMax {
        Name("pMax"),
        Comment("Maximum momentum value [GeV/c] in likelihood scan."),
        7.50 };
      fhicl::Atom<double> pStep {
        Name("pStep"),
        Comment("Step in momentum value [GeV/c] in likelihood scan."),
        0.01 };
    ...
    explicit TrajectoryMCSFitterUBoone(
      const Parameters& p) : TrajectoryMCSFitterUBoone(
        p().pIdHypothesis(),
        p().minNumSegments(),
        p().minNumAngles(),
        p().segmentLength(),
    ...
    const ScanResult doLikelihoodScan(
    ...
    inline double MomentumDependentConstant(
