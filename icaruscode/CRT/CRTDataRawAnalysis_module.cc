...
namespace crt {
  ...
  class CRTDataRawAnalysis : public art::EDAnalyzer
  {
  public:
    ...
    explicit CRTDataRawAnalysis(Parameters const& config);
  ...
  map<int,vector<pair<int,int>>> CRTDataRawAnalysis::fFebMap;
 
  CRTDataRawAnalysis::CRTDataRawAnalysis(Parameters const& config)
    : EDAnalyzer(config)
  ...
  void CRTDataRawAnalysis::FillFebMap() { 
  ...
  //-----------------------------------------------------------------------
  void CRTDataRawAnalysis::beginJob()
  {
     mf::LogError("CRTDataRawAnalysis") << " starting analysis job" << std::endl;
    ...
    fDAQNtuple->Branch("counterT0",		&fTs0,		     "ts0/i");
    fDAQNtuple->Branch("counterT1",		&fTs1,		     "ts1/i");
    fDAQNtuple->Branch("full_second",		&fFullSecond,	     "fullSecond/l");
  ...
  void CRTDataRawAnalysis::beginRun(const art::Run&)
  {
  }

  //-----------------------------------------------------------------------
  void CRTDataRawAnalysis::analyze(const art::Event& event) 
  {
    MF_LOG_DEBUG("CRTDataRawAnalysis") << "beginning analyis" << '\n';
    ...
    art::Handle<vector<icarus::crt::CRTDataRaw>> crtDAQHandle;
    vector<art::Ptr<icarus::crt::CRTDataRaw> > crtList;
    if ( event.getByLabel(fCRTDAQProducerLabel, crtDAQHandle))
      art::fill_ptr_vector(crtList, crtDAQHandle);
    
    vector<art::Ptr<icarus::crt::CRTDataRaw>> crtData;
    bool presel = false;
    ...
    for (size_t febdat_i=0; febdat_i<crtData.size(); febdat_i++) {
      ...
      fTs0 = crtData[febdat_i]->fCounterT0;
      std::cout<<"AAA  "<<fTs0<<std::endl;

      fTs1 = crtData[febdat_i]->fCounterT1;
      fFullSecond = crtData[febdat_i]->fFullSecond;
    ...
    if (isCRTHit) {
       ...
       mf::LogError("CRTDataRawAnalysis") << "looping over reco hits..." << std::endl;
	  ...
	  if (ittmp==hit.pesmap.end()) {
	     mf::LogError("CRTDataRawAnalysis") << "hitreg: " << fHitReg << std::endl;
	     mf::LogError("CRTDataRawAnalysis") << "fHitSubSys: "<< fHitSubSys << std::endl;
	     mf::LogError("CRTDataRawAnalysis") << "mactmp = " << mactmp << std::endl;
	     mf::LogError("CRTDataRawAnalysis") << "could not find mac in pesmap!" << std::endl;
	    continue;
	  }	  
    ...
    else  mf::LogError("CRTDataRawAnalysis") << "CRTHit products not found! (expected if decoder step)" << std::endl;


  } // CRTDataAnalysis::analyze()
  
  
  DEFINE_ART_MODULE(CRTDataRawAnalysis)
