...
void icarus::crt::CRTT0Tagging::produce(art::Event& e)
{
  ...
  std::unique_ptr< art::Assns<recob::PFParticle, anab::T0> > pfpAssn( new art::Assns<recob::PFParticle, anab::T0>);
  ...
  for(const auto& PFPLabel : fPFParticleLabel) {
    ...
    for (art::Ptr<recob::PFParticle> p_pfp: PFParticleList) {
      ...
      if(bestCrtCand.distance<=fGoodCandidateDistance){ 
       ...
       std::cout 	      <<"Matched CRT time = "<<bestCrtCand.CRThit.ts1_ns/1e3<<" [us] to track "<<track.ID()<<" with projection-hit distance = "<<bestCrtCand.distance<<" Track T0 "<<t0
	      <<"\nMatched CRT hit plane: "<<bestCrtCand.CRThit.plane<<" xpos "<<bestCrtCand.CRThit.x_pos<<" ypos "<<bestCrtCand.CRThit.y_pos<<" zpos "<<bestCrtCand.CRThit.z_pos << std::endl;
        ...
        util::CreateAssn(*this, e, *t0col, p_pfp, *pfpAssn);
