...
void icarus::crt::CRTT0Tagging::produce(art::Event& e)
{
  ...
  for(const auto& [ PFPLabel, TPCTrackLabel, HitLabel, TRKHMLabel ]: util::zip(fPFParticleLabel, fTPCTrackLabel, fHitLabel, fTRKHMLabel)){
    ...
    if(!simchannels.empty() && !fData){
      ...
      id_to_ide_map = RecoUtils::PrepSimChannels(simchannels, *wireReadout);
    ...
    for (art::Ptr<recob::PFParticle> const& p_pfp: PFParticleList) {
      ...
      for(art::Ptr<CRTHit> const& p_crthit: CRTHitList){
        ...
        icarus::crt::DriftedTrack thisDriftedTrack = fMatchingAlg.DriftTrack(trkHits, trkHitMetas, fGeometryService, wireReadout, detProp, clockData, crtTime, track, 0);    
      ...
      if(bestCrtCand.distance<=fGoodCandidateDistance){ 
        ...
        icarus::crt::DriftedTrack thisMatchedDriftedTrack = fMatchingAlg.DriftTrack(trkHits, trkHitMetas, fGeometryService, wireReadout, detProp, clockData, bestCrtCand.CRThit.ts1_ns/1e3, track, 0);
        ...
        sbn::crt::CRTHitT0TaggingInfo matchInfo {bestCrtCand.distance, matchedSys, bestCrtCand.CRThit.plane, bestCrtCand.CRThit.ts1_ns, bestCrtCand.delta.X(), bestCrtCand.delta.Y(), bestCrtCand.delta.Z(), bestCrtCand.crossPoint.X(), bestCrtCand.crossPoint.Y(), bestCrtCand.crossPoint.Z(), bestCrtCand.plane, driftedMatchedPCAResults.eigenVector1, trackFit, matchMethod};        
