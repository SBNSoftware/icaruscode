...
namespace crt {
  ...
  void CRTSimAnalysis::analyze(const art::Event& event) 
  {
    ...
    //if (!event.getByLabel("genericcrt", auxDetSimChannelHandle)) {
        throw cet::exception("CRTSimAnalysis")
    ...
    std::cout << "Geo NAuxDets=" << fGeometryService->NAuxDets() << std::endl;
    for (size_t ai=0;ai<fGeometryService->NAuxDets();ai++) {
      auto ad = fGeometryService->AuxDet(ai);
