...
proc  = subprocess.Popen('cd %s;lar -c fakenews_driver.fcl' % WORK_DIR,shell=True,stderr=subprocess.STDOUT,stdout=subprocess.PIPE)
state = None
time_slept = 0
while state is None:
    state = proc.poll()
...
if not state == 0:
    ...
    os.system('hadd %s %s/potus.root %s/impeach.root' % (OUT_FILE,WORK_DIR,WORK_DIR))
    print
