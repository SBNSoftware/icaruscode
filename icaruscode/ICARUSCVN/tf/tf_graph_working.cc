                 ...
                 int noutputs)
{
  ...
  if (fUseBundle) {
    ...
    // }
    if (outputs.empty()){
      for (int i = n_inputs; i < graph_def.node().size(); i++){
        std::cout << "  key: " << graph_def.node()[i].name() << std::endl;
        fOutputNames.push_back(graph_def.node()[i].name());
      }
    }
    else // or last nodes with names containing provided strings
    {
      ...
      for (size_t n = 0; n < ng; ++n) {
        ...
        if (found) {
          if (!last.empty() ) { fOutputNames.push_back(last); } //&& (basename != current) (estava no ifs)
          current = basename;
          last = name;
          std::cout << "base: " << basename << " last: " << last << std::endl;
        }
      }
    ...
    if (fOutputNames.empty()) {
      ...
      for (auto &k : fOutputNames) std::cout << "Output: " << k << std::endl;
    }
