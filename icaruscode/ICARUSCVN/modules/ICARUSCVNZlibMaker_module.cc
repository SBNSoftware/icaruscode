...
// #include "../module_helpers/ICARUSICVNZlibMaker.h"
#include "icaruscode/ICARUSCVN/module_helpers/ICARUSICVNZlibMaker.h"
...
namespace lcvn {
  ...
  class ICARUSCVNZlibMaker : public ICARUSICVNZlibMaker {
  public:

    explicit ICARUSCVNZlibMaker(fhicl::ParameterSet const& pset);
    ~ICARUSCVNZlibMaker();
  ...
  private:
    ...
    double fT0;
    std::string fZlibOutputName;
    std::string fInfoOutputName;
  };

  //......................................................................
  ICARUSCVNZlibMaker::ICARUSCVNZlibMaker(fhicl::ParameterSet const& pset)
    : ICARUSICVNZlibMaker(pset)
  {
    reconfigure(pset);
  }

  //......................................................................
  ICARUSCVNZlibMaker::~ICARUSCVNZlibMaker()
  {  }

  //......................................................................
  void ICARUSCVNZlibMaker::reconfigure(const fhicl::ParameterSet& pset)
  {
    std::cout << "Reconfigure ZlibMaker" << std::endl;
    ICARUSICVNZlibMaker::reconfigure(pset);
    ...
    fZlibOutputName = pset.get<std::string>("ZLibFileName");
    fInfoOutputName = pset.get<std::string>("InfoFileName");
  }

  //......................................................................
  void ICARUSCVNZlibMaker::endSubRun(const art::SubRun & sr){
    std::cout << "EndSubRun" << std::endl;
  ...
  void ICARUSCVNZlibMaker::beginJob()
  {
    std::cout << "Begin Job ZlibMaker" << std::endl;
    ICARUSICVNZlibMaker::beginJob();
  ...
  void ICARUSCVNZlibMaker::analyze(const art::Event& evt)
  {
    std::cout << "Begin Analyze ZlibMaker" << std::endl;	
    ...
    if(fUseSlice){
      ...
      if(fUseBackTrackInfo){
        ...
        for(unsigned int i=0; i<pixelmaps.size(); i++){
          ...
          if (findOneSlice.isValid()){
            ...
            std::cout << "Evt run: " <<  evt.run() << "  Evt subRun: " << evt.subRun() << "  Evt event: " << evt.event() << "  Slice ID: " << slice->ID() << "\n";
            AssignLabels labels;
            TDNuInfo info;
            double event_weight =1;
            InteractionType interaction = kOther;
            ...
            if(findManyHits.isValid()){
              std::cout << "Found many hists at: " << slice.key() << std::endl;
		  ...
		  std::cout << "0. Many PFPs is valid" << std::endl;
            ...
            if(findManyPFPs.isValid()){
	            std::cout << "1. Many PFPs is valid" << std::endl;
            ...
            std::cout << "0. Writing files" << std::endl;
        ...
        for(unsigned int i=0; i<pixelmaps.size(); i++){
          ...
          if (findOneSlice.isValid()){
            ...
            //}
            std::string evtid = "r"+std::to_string(evt.run())+"_s"+std::to_string(evt.subRun())+"_e"+std::to_string(evt.event())+"_sl"+std::to_string(slice->ID())+"_h"+std::to_string(time(0));
            std::cout << "1. Writing files" << std::endl;
	          write_files(train, evtid);
          }
      ...
      std::string evtid = "_e"+std::to_string(evt.event());
      std::cout << "2. Writing files" << std::endl;
      write_files(train, evtid);
  ...
  void ICARUSCVNZlibMaker::write_files(LArTrainingNuData td, std::string evtid)
  {
    std::string fileName = "";
    ...
    // std::cout << "Wires: " << td.fPMap.NWire() << " tdc: " << td.fPMap.NTdc() << " pixel array size: " << pixel_array.size()  << std::endl;
    // std::cout << "n pixel: " << td.fPMap.NPixel() << "bound: " << td.fPMap.Bound() << " tothits: " << td.fPMap.GetTotHits() << std::endl;
    
    //USE THIS IF BREAKING ON MINMAXTDCS	
    //fImage.DisableRegionSelection();
    fImage.SetViewReversal(false, false, false);
    ...
    // Compression ok 
      ...
      // Create output files 
      // std::string image_file_name = "event_" + evtid + ".gz";
      // std::string info_file_name = "event_" +  evtid + ".info";

      std::random_device rd;                         // Non-deterministic seed (if available)
      std::mt19937 gen(rd());                        // Mersenne Twister engine
      std::uniform_int_distribution<> dist1(0, 999);
      std::uniform_int_distribution<> dist2(0, 1999999);
      // Generate numbers
      int r1 = dist1(gen);
      int r2 = dist2(gen);
      std::string dir = fGrid ? "" : "/";
      std::string image_file_name = fOutputDir + dir + fZlibOutputName + fileName + "_" + evtid + "_" + fModule + "_" + std::to_string(r1) + "_" + std::to_string(r2) + ".gz";
      std::string info_file_name = fOutputDir + dir + fInfoOutputName + fileName + "_" + evtid + "_" + fModule + "_" + std::to_string(r1) + "_" + std::to_string(r2) + ".info";
      
      std::ofstream image_file (image_file_name, std::ofstream::binary);
      std::ofstream info_file  (info_file_name);

      std::cout << fZlibOutputName << " - file name: " << image_file_name << std::endl;
      ...
      if(image_file.is_open() && info_file.is_open()) {
        ...
        if (image_file.is_open())
          ...
          throw art::Exception(art::errors::FileOpenError)
            << "Unable to open file " << fZlibOutputName.c_str() << "!" << std::endl;

        if (info_file.is_open())
          info_file.close();
        else
          throw art::Exception(art::errors::FileOpenError)
            << "Unable to open file " << fInfoOutputName.c_str() << "!" << std::endl;
      }
    }
  ...
  bool ICARUSCVNZlibMaker::Is_in_Fiducial_Vol(double stX, double stY, double stZ)
  {
    //if(fverbose) std::cout << "Is in Fiducial Vol Starting" << std::endl;
    ...
    //USE ABS FOR X 
    double X_low_bound = 86.94;
    double X_up_bound = 333.49;
    double Y_low_bound = -156.86;
    double Y_up_bound = 109.96;
    double Z_low_bound = -864.95;
    double Z_up_bound = 844.95;

    if((TMath::Abs(stX) >= X_low_bound && TMath::Abs(stX) <= X_up_bound) && 
    (stY >= Y_low_bound && stY <= Y_up_bound) && (stZ >=Z_low_bound && stZ <= Z_up_bound)) return true;
    else return false;
  }  

  /////////////////////////////////////////////////////////////////////////////////////////////////////////

  void  ICARUSCVNZlibMaker::HitTruth( detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit, Int_t& truthid){
    //if(fverbose) std::cout << "HitTruth Starting" << std::endl;
  ...
  bool  ICARUSCVNZlibMaker::HitTruthId(detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit, Int_t& mcid){
    //if(fverbose) std::cout << "HitTruthId Starting" << std::endl;
  ...
  bool  ICARUSCVNZlibMaker::TrackIdToMCTruth(Int_t const trkID, art::Ptr<simb::MCTruth>& mctruth){
    bool matchFound = false;
    //if(fverbose) std::cout << "TrackIdToMCTruth Starting" << std::endl;
    try {
  ...
  double ICARUSCVNZlibMaker::HitEfrmTrkID(detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit, Int_t truthid){
    //if(fverbose) std::cout << "HitEfrmTrkID Starting" << std::endl;
    double hit_E = 0.;
    std::vector<sim::TrackIDE> trackIDEs = bt_serv->HitToTrackIDEs(clockData, hit);
  ...
  double ICARUSCVNZlibMaker::HitTotE(detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit){
    //if(fverbose) std::cout << "HitTotE Starting" << std::endl;
    double Tot_E = 0.;
    std::vector<sim::TrackIDE> trackIDEs = bt_serv->HitToTrackIDEs(clockData, hit);
  ...
  bool ICARUSCVNZlibMaker::Is_Slice_Nu(art::FindManyP<recob::PFParticle> const& assn, art::Ptr<recob::Slice> const& slce){
    //if(fverbose) std::cout << "Is_Slice_Nu Starting" << std::endl;
    if(assn.isValid()){
  ...
  double ICARUSCVNZlibMaker::Get_Slice_Score(art::FindManyP<larpandoraobj::PFParticleMetadata> const& assn, art::Ptr<recob::PFParticle> const& pfp){
    //if(fverbose) std::cout << "Get_Slice_Score Starting" << std::endl;
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  art::Ptr<recob::PFParticle> ICARUSCVNZlibMaker::Get_Nu_like_PFP(art::FindManyP<recob::PFParticle> const& assn, art::Ptr<recob::Slice> const& slce){
  ...
  std::vector<int> ICARUSCVNZlibMaker::Get_Best_Slice_ID_PFP_pdg(art::FindManyP<recob::PFParticle> const& pfp_assn, art::FindManyP<larpandoraobj::PFParticleMetadata> const& metadata_assn,
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
  int ICARUSCVNZlibMaker::Get_Slice_PFP_ID(art::FindManyP<recob::PFParticle> const& assn, art::Ptr<recob::Slice> const& slce){
    //if(fverbose) std::cout << "Get_Slice_PFP_ID Starting" << std::endl;
    std::vector<art::Ptr<recob::PFParticle>> slicePFPs = assn.at(slce.key());
    int result = -9999;
    for(auto const &pfp : slicePFPs){
      if((pfp->PdgCode() == 12 || pfp->PdgCode() == 14)){ 
  ...
  int ICARUSCVNZlibMaker::Get_Hit_Count(unsigned int tpc_no, unsigned int pln_no, std::vector<art::Ptr<recob::Hit>> const& hit_vec){
    //if(fverbose) std::cout << "Get_Hit_Count Starting" << std::endl;
    int n_hits = 0;
    if(hit_vec.size()){
  ...
  double ICARUSCVNZlibMaker::Get_Tot_nu_E(detinfo::DetectorClocksData const& clockData,art::Ptr<simb::MCTruth> const& mcneutrino, std::vector<art::Ptr<recob::Hit>> const& hit_vec){
    double tot_nu_E = 0;
    for(auto const hit : hit_vec){
      Int_t trkId;
  ...
  double ICARUSCVNZlibMaker::HitTotE_frm_given_orgin(detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit, std::string origin_name){
    double Tot_E = 0.;
    std::vector<sim::TrackIDE> trackIDEs = bt_serv->HitToTrackIDEs(clockData, hit);
    if( !trackIDEs.size() ) return Tot_E;
    for(size_t i = 0; i < trackIDEs.size(); ++i){
  ...
  double ICARUSCVNZlibMaker::HitTotE_frm_mctruth(detinfo::DetectorClocksData const& clockData, art::Ptr<recob::Hit> const& hit, art::Ptr<simb::MCTruth> const& my_mctruth){
    double Tot_E = 0.;
    std::vector<sim::TrackIDE> trackIDEs = bt_serv->HitToTrackIDEs(clockData, hit);
    if( !trackIDEs.size() ) return Tot_E;
    for(size_t i = 0; i < trackIDEs.size(); ++i){
  ...
  double ICARUSCVNZlibMaker::TotE_from_mctruth(detinfo::DetectorClocksData const& clockData, std::vector<art::Ptr<recob::Hit>> const& hit_vec, art::Ptr<simb::MCTruth> const& my_mctruth){
    double tot_E = 0;
  ...
  void ICARUSCVNZlibMaker::Clear(){
    if(fverbose) std::cout << "Clear ZlibMaker " << std::endl;
    frun = -9999;
    fsubrun = -9999;
    fevent = -9999;
    fsliceID = -9999;
  ...
  DEFINE_ART_MODULE(ICARUSCVNZlibMaker)
} // namespace cvn
