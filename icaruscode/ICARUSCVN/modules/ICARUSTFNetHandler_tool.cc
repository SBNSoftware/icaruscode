...
ICARUSTFNetHandler::ICARUSTFNetHandler(const fhicl::ParameterSet &pset)
  ...
  cet::search_path sp("FW_SEARCH_PATH");   // read environment variable
  std::string TFProtoBuf = sp.find_file(fTFProtoBuf.c_str());  // relative path

  mf::LogInfo("TFNetHandler")
      << "Loading network: " << TFProtoBuf << std::endl;
  fTFGraph = tf::Graph::create(TFProtoBuf.c_str(), fInputs, fOutputs,
