...
double PulseStartExtractor::extractStart(const std::vector<T> &wf)
{
  ...
  if (fMethod == SIGMOID_FIT)
  {
    double par[4] = {0};
    bool ok = extractStartSigmoidFit(wf, cf_sample, par);
    if (ok)
      return par[0]; // Rising edge time from the sigmoid fit (can be non-integer).
    else
...
template <typename T>
bool PulseStartExtractor::extractStartSigmoidFit(const std::vector<T> &wf, std::size_t guess, double par[4])
{
  ...
  std::string sigmoidModel = "1/(1+exp(([0]-x)/[1]))*[2]+[3]";
  TF1 *fitFunc = new TF1("sigmoid", sigmoidModel.c_str(), x[llim], x[ulim]);
...
template bool PulseStartExtractor::extractStartSigmoidFit<short>(const std::vector<short>& wf, std::size_t guess, double par[4]);
template bool PulseStartExtractor::extractStartSigmoidFit<int>(const std::vector<int>& wf, std::size_t guess, double par[4]);
template bool PulseStartExtractor::extractStartSigmoidFit<float>(const std::vector<float>& wf, std::size_t guess, double par[4]);
template bool PulseStartExtractor::extractStartSigmoidFit<double>(const std::vector<double>& wf, std::size_t guess, double par[4]);
