...
void KeyedCSVparser_documentation_test() {
   ...
   * parser.addPatterns({
   *       { std::regex{"Trigger Type"}, 1U } // expect one value (even if contains letters)
   *     , { std::regex{"TriggerWindows"}, 1U } // expect one value (even if contains letters)
   *     , { std::regex{"TPChitTimes"}, icarus::details::KeyedCSVparser::FixedSize }
  ...
  icarus::details::KeyedCSVparser parser;
  parser.addPatterns({
        { std::regex{"TriggerType"}, 1U } // expect one value (even if contains letters)
      , { std::regex{"TriggerWindows"}, 1U } // expect one value (even if contains letters)
      , { std::regex{"TPChitTimes"}, icarus::details::KeyedCSVparser::FixedSize }
           // the first value is an integer, count of how many other values
    });
